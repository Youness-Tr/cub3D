...
/*   Updated: 2024/11/28 10:46:54 by ajabri           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_mini_2d(t_cub *cub)
{
	...
	factor = (float)(cub->var.s_w * MINI_MAP) / cub->map.map_w + 1;
		...
		while (i < cub->map.map_w + 1)
		{
			if (cub->map.map2d[j][i] == '1')
				render_square(&cub->img, (i * TILE_SIZE) * MINI_MAP, (j
						* TILE_SIZE) * MINI_MAP, factor, 0x000000);
			else if (cub->map.map2d[j][i] == '0' || cub->map.map2d[j][i] == 'P')
			{
				render_square(&cub->img, (i * TILE_SIZE) * MINI_MAP, (j
						* TILE_SIZE) * MINI_MAP, factor, 0xffffff);
			}
			else if (cub->map.map2d[j][i] == 'D')
			{
				render_square(&cub->img, (i * TILE_SIZE) * MINI_MAP, (j
						* TILE_SIZE) * MINI_MAP, factor, 0x00FF00);
			}
			i++;
		}
		j++;
	}
	render_circle(&cub->img, cub->plyr.plyr_x * MINI_MAP, cub->plyr.plyr_y
		* MINI_MAP, PLAYER_RADIUS * MINI_MAP, 0xFF0000);
