...
/*   Updated: 2024/09/15 10:03:32 by ajabri           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_mini_malloc(char **ptr, char *s, char c, t_leak *leak)
{
	size_t	i;
	size_t	len;
	...
	while (*s)
	{
		if (*s != c)
		{
			...
			while (*s && *s != c && ++len)
				s++;
			ptr[i] = ft_substrv2(s - len, 0, len, leak);
			if (!ptr[i])
			{
